...
SbTab_t *Program(Sytree_t *sytree)
{
	...
	//write 函数
	SbTab_t *funcSy = AllocSymbol("write",FUNCDEF);
	FuncTab_t *func =  (FuncTab_t*)malloc(sizeof(FuncTab_t));
	memset(func,0,sizeof(FuncTab_t));
	SbTab_t *retT = AllocSymbol("int",INT_T);
	func->retType =retT;
	char*t;
    t=(char*)malloc(sizeof(char )*(strlen("write")+1));
	strcpy(t,"write");
	func->name=t;
	func->hasDef =1;
	func->FuncDef = NULL;
	SbTab_t *paralist = AllocSbTab(ParaListTabHeader);
	SbTab_t *para = AllocSymbol("a",INT_T);
	InsertSymbol(&paralist,para);
	func->paraList = paralist;
	funcSy->SubTab = func;
	setFunParent(funcSy,&func);
	InsertSymbol(&gTab,funcSy);
...
SbTab_t *ExtDef(SbTab_t **tab, TabTyp_t tabtyp,node_t *node)
{
	...
	if(strcmp(child->name,"ExtDecList")==0)
	{
		...
		return ExtDecList(tab,tabtyp,child,type);
	}
	...
	else //函数定义 或声明
	{
		...
		{//ExtDef: Specifier FunDec SEMI
		 ...
		 //      | ID LP RP
			FuncTab_t *fun=FunDec(tab,tabtyp, child,type);
			SbTab_t *sb = NULL;
...
SbTab_t *ArrayDec(SbTab_t **sy,SbTab_t *pre,int depth,node_t *node,SbTab_t *t)
{
	int children=haschildren(node);
	if(children==-1)
		return NULL;
	...
	else
	{
		...
		sprintf(tmpi,"%d",depth);
		char tmp[20]={'a','\0'};
		strcat(tmp,tmpi);
		SbTab_t *a=AllocSymbol(tmp,t->SbType);
		a->arraySize=node->child->ptr[2]->I_type;
		if(depth ==1)
		{
			a->SbType = t->SbType;
			a->SubTab=t->SubTab;
			// SbTab_t **tmp = &(t->SubTab);
			// (*tmp)->refcount ++;
		}
		else
		{
		...
		ArrayDec(sy,a,depth+1,node->child->ptr[0],t);
	}
	return *sy;
}
...
SbTab_t *Exp(SbTab_t **tab,TabTyp_t tabtyp,node_t *node)
{
	...
	{//Exp : Exp xxx
		if(children==3)
		{
			...
			else if( strcmp(childs[1]->name,"ADD")==0 || strcmp(childs[1]->name,"SUB")==0 || strcmp(childs[1]->name,"MUL")==0 || strcmp(childs[1]->name,"DIV")==0 ||  strcmp(childs[1]->name,"RELOP")==0)
				// | Exp PLUS Exp
				// | Exp MINUS Exp
				// | Exp STAR Exp
				// | Exp DIV Exp
			...
			else if( strcmp(childs[1]->name,"AND")==0 || strcmp(childs[1]->name,"OR")==0 )
				// Exp: Exp AND Exp
				//    | Exp OR Exp
			{
				lvar = Exp( tab,tabtyp,childs[0]);
				...
				if( lvar==NULL || rval==NULL )
					return NULL;
				//if(lvar->SbType==rval->SbType && (lvar->SbType == FLOAT_T || lvar->SbType == INT_T))
				if(typeCmp(lvar,rval)==0 && (lvar->SbType == INT_T))
					return lvar;
				else{
					printf( "Error type 7 at line %d: Type mismatched for operands.\n", childs[0]->lineno );
					errorflag ++;
	...
	else if(strcmp(childs[0]->name,"ID")==0)
	{
		...
		{// Exp: ID LP (Args) RPs
			if(strcmp(childs[0]->ID_type,"read")==0 || strcmp(childs[0]->ID_type,"write")==0)
			{
				return NULL;
			}
			else
	...
	{
		SbTab_t *rval = Exp(tab,tabtyp,childs[1]);
	...
	{
		return Exp(tab,tabtyp,childs[1]);
	}
...
int typeCmp(SbTab_t *a, SbTab_t *b)
{
	...
	if(a!=NULL && b!=NULL)
	{
		...
		switch(a->SbType)
		{
			case ARRAY:return arraytypeCmp((SbTab_t*)a->SubTab,(SbTab_t*)b->SubTab);break;
...
int arraytypeCmp(SbTab_t *a, SbTab_t*b)
{
	...
	if(a!=NULL && b!=NULL)
	{
		if(a->SbType!=b->SbType)
			return -1;
		if(a->SbType==b->SbType)
			if(a->SubTab==NULL && b->SubTab==NULL)
				return 0;
			return arraytypeCmp(a->SubTab,b->SubTab);
	}	
}
