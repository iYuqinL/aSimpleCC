/*************************************************************************
	...
	> Created Time: Tue 03 Jul 2018 09:43:58 PM CST
...
void stVar(VarDes* var, FILE* fp){
	...
	else if(var->op->kind == VARIABLE_OP || var->op->kind == TEMPVAR_OP || var->op->kind==VARDEC_OP){
		memset(stCode, 0, sizeof(stCode));
...
void ldVar(VarDes* var, FILE* fp)
{
	...
	else if(var->op->kind == CONSTANT_OP){
		memset(ldCode, 0, sizeof(ldCode));
		sprintf(ldCode, "li $%s, %s\n", regList[var->regNo].name, var->op->u.value);
	}
	else if(var->op->kind == ADDRESS_OP){
		int addrReg = allocateRegForOp(var->op->u.addr, fp);
		memset(ldCode, 0, sizeof(ldCode));
...
int allocateRegForOp(Operand_t *op, FILE* fp)
{
	...
	if(var == NULL){
		isFirstUsed = TRUE;
		var = (VarDes*) malloc(sizeof(VarDes));
		memset(var, 0, sizeof(VarDes));
		var->op = op;
		var->regNo = -1;
		if(var->op->kind == VARIABLE_OP || var->op->kind == TEMPVAR_OP){
			offset += 4;
			var->offset = offset;
		}
	...
	if(var->regNo < 0){
		...
		if(var->op->kind == CONSTANT_OP||var->op->kind == ADDRESS_OP||(isFirstUsed==FALSE && var->offset>=0)){
			//不是第一次出现的变量 
			ldVar(var, fp);
		}
	}
...
void fwriteOneObjCode(InterCode_t* ir, FILE* fp)
{
	...
	switch(ir->kind){
		...
		case ASSIGN_CODE:{
			Operand_t* leftOp = ir->u.assign.left;
			Operand_t* rightOp = ir->u.assign.right;
		...
		case DIV_CODE:{
			...
			fputs(str, fp);
			if(ir->u.doubleop.result->kind == ADDRESS_OP)
		...
		case RETURN_CODE:{
			Operand_t* retOp = ir->u.singleop.op;
			int retReg;
			...
			char str[64];
			memset(str, 0, sizeof(str));
			if(retOp->kind==VARIABLE_OP || retOp->kind==TEMPVAR_OP)
				sprintf(str, "move $v0, $%s\n", regList[retReg].name);
			else if(retOp->kind==CONSTANT_OP)
				sprintf(str, "li $v0, %s\n", retOp->u.value);
			fputs(str, fp);
			fputs("jr $ra\n", fp);
			break;
		...
		case DEC_CODE:{
			...
			// 根据这一特性可以进行优化
			// 从而节省指令
			assert(ir->nextCode->kind == ADDR_CODE);
